...
// -----------------------------------------------------------------------------
// FIX-83 / W122 forward-regression SOURCE-LEVEL guard.
//
// These tests don't exercise behavior — they grep the source file. They exist
// to fail loudly if a future drive-by refactor reverts the MSB-first codec to
// LSB-first packing, OR re-adds the prose "vectors were generated by an older
// build of Bitcoin Core" rationalization. The behavioral tests above
// (TestW122_*, TestBIP158Vectors) would also catch a real regression, but
// these source-guards make the intent explicit: the W122 bugs MUST stay fixed,
// and the W122 comment-as-confession MUST stay gone.
// -----------------------------------------------------------------------------

// readSourceFile loads a file from this package's directory by name.
...
// TestW122_SourceGuard_MSBFirstIdiomPresent asserts the new MSB-first
// codec lives in blockfilterindex.go. We grep for the specific shift
// expression that Core's BitStreamWriter::Write uses — `data << (64 -
// remaining)` (Go transliteration of Core's `data << (64 - nbits)`).
// If a future refactor goes back to plain `v << w.numBits` (LSB-first),
// this test fails with a load-bearing message.
func TestW122_SourceGuard_MSBFirstIdiomPresent(t *testing.T) {
	src := readSourceFile(t, "blockfilterindex.go")
	// Core's MSB-first shift idiom translated to Go's local-var names.
	mustContain := []string{
		"v << (64 - remaining)",
		"shifted >>= 64 - 8 + w.offset",
		"uint8(br.buffer<<br.offset) >> (8 - bits)",
	}
	for _, needle := range mustContain {
		if !strings.Contains(src, needle) {
			t.Errorf("FIX-83 / W122 source-guard: blockfilterindex.go missing MSB-first idiom %q. "+
				"If this fired, someone reverted the bitStreamWriter/Reader rewrite. "+
				"See bitcoin-core/src/streams.h:329-358 BitStreamWriter::Write for the reference.",
				needle)
		}
	}
}

// TestW122_FormatMigration_FromPreFIX83 simulates an existing operator
// who upgrades from a pre-FIX-83 build: state-row uses the OLD 68-byte
// layout (no FormatVersion byte) and filter rows are LSB-first bytes.
// On Init, the index must detect the old layout, wipe stale rows, and
// reset BestHeight to -1 so the IndexManager rebuilds with MSB-first
// codec on the next chain catchup.
func TestW122_FormatMigration_FromPreFIX83(t *testing.T) {
	db := NewMemDB()
	defer db.Close()

	// Forge the pre-FIX-83 state row: 4 + 32 + 32 = 68 bytes (no version).
	preFIX83State := make([]byte, 68)
	// int32 LE BestHeight = 5
	preFIX83State[0] = 5
	preFIX83State[1] = 0
	preFIX83State[2] = 0
	preFIX83State[3] = 0
	// 32-byte block hash + 32-byte prev filter header — fill with sentinels
	for i := 4; i < 36; i++ {
		preFIX83State[i] = 0xAA
	}
	for i := 36; i < 68; i++ {
		preFIX83State[i] = 0xBB
	}
	if err := db.Put(BlockFilterStateKey, preFIX83State); err != nil {
		t.Fatalf("seed pre-FIX-83 state: %v", err)
	}

	// Forge two stale filter rows under BlockFilterPrefix.
	for _, h := range []int32{0, 1} {
		key := MakeBlockFilterKey(h)
		if err := db.Put(key, []byte{0xDE, 0xAD, 0xBE, 0xEF}); err != nil {
			t.Fatalf("seed stale row at %d: %v", h, err)
		}
	}

	// Init must NOT error and must reset to -1.
	idx := NewBlockFilterIndex(db)
	if err := idx.Init(); err != nil {
		t.Fatalf("Init from pre-FIX-83 should not error: %v", err)
	}
	if idx.BestHeight() != -1 {
		t.Errorf("post-migration BestHeight = %d, want -1 (rebuild)", idx.BestHeight())
	}

	// Stale rows must be wiped.
	for _, h := range []int32{0, 1} {
		key := MakeBlockFilterKey(h)
		data, err := db.Get(key)
		if err != nil {
			t.Errorf("stale-row read at %d: %v", h, err)
			continue
		}
		if data != nil {
			t.Errorf("stale row at height %d not wiped: %x", h, data)
		}
	}

	// Fresh write — must use FormatVersion 1.
	block := createTestBlockWithScripts(0)
	hash := block.Header.BlockHash()
	if err := idx.WriteBlock(block, 0, hash, nil); err != nil {
		t.Fatalf("WriteBlock after migration: %v", err)
	}
	freshState, err := db.Get(BlockFilterStateKey)
	if err != nil {
		t.Fatalf("read state after WriteBlock: %v", err)
	}
	if len(freshState) != 69 {
		t.Errorf("post-migration state row length = %d, want 69 (1+4+32+32)", len(freshState))
	}
	if freshState[0] != BlockFilterIndexFormatVersion {
		t.Errorf("post-migration state row FormatVersion = %d, want %d",
			freshState[0], BlockFilterIndexFormatVersion)
	}

	// Re-init: must NOT trigger migration this time (we just wrote v1).
	idx2 := NewBlockFilterIndex(db)
	if err := idx2.Init(); err != nil {
		t.Fatalf("Init after fresh write: %v", err)
	}
	if idx2.BestHeight() != 0 {
		t.Errorf("re-Init BestHeight = %d, want 0", idx2.BestHeight())
	}
}

// TestW122_SourceGuard_CommentAsConfessionGone asserts the prose
// rationalization at the old blockfilterindex_test.go:541-554 location
// is gone. The old comment claimed Core's blockfilters.json was stale
// and used that as cover for skipping byte-exact assertions; that claim
// was factually wrong (Core BOOST_CHECKs the JSON byte-exactly in
// blockfilter_tests.cpp:172 every CI build) and masked W122 BUG-2.
func TestW122_SourceGuard_CommentAsConfessionGone(t *testing.T) {
	src := readSourceFile(t, "blockfilterindex_test.go")
	// Phrases from the deleted "vectors are stale" rationalization.
	mustNotContain := []string{
		"vectors were generated by an older build",
		"does NOT compare filter bytes against the JSON",
	}
	for _, needle := range mustNotContain {
		if strings.Contains(src, needle) {
			t.Errorf("FIX-83 / W122 source-guard: blockfilterindex_test.go still carries "+
				"comment-as-confession phrase %q. The W122 BUG-2 byte-exact assertion was "+
				"deliberately disabled by that comment block; removing the bug requires "+
				"removing the rationalization too.", needle)
		}
	}
}
