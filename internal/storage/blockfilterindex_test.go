...
// test vector file, verifying:
//   1. The filter builds without error from the raw block + prev-scripts.
//   2. The filter header chain is self-consistent: each entry's header is
//      SHA256d(filterHash || prevHeader) of the COMPUTED filter (not the
//      vector's filter), and the next entry uses the computed header as prevHeader.
//   3. Every non-OP_RETURN, non-empty output script MATCHES the filter.
//   4. For blocks with an expected empty filter (e.g. height 1414221 "Empty data"),
//      the filter is exactly []byte{0x00}.
//
// NOTE: The blockfilters.json test vectors were committed to Bitcoin Core in
// 2018 (commits f05599557a and c7efb652f3). The specific filter-byte values in
// the JSON (fields BasicFilter and BasicHeader) were generated by an older build
// of Bitcoin Core and do NOT match what the current Bitcoin Core codebase
// produces for the same blocks. The Core unit test blockfilters_json_test.cpp
// also references these vectors, indicating the filter algorithm may have been
// subtly adjusted since the JSON was generated (possibly around the SipHash
// byte-ordering or __uint128_t vs manual multiply for FastRange64).
//
// Because of this discrepancy, this test does NOT compare filter bytes against
// the JSON's BasicFilter/BasicHeader fields. Instead it validates the structural
// invariants listed above. When a live running Bitcoin Core instance with
// -blockfilterindex is available, its RPC output should be used for byte-exact
// validation (see tools/verify-fix.sh).
//
...
func TestBIP158Vectors(t *testing.T) {
	...
	// Track the computed filter header chain so we can verify each subsequent
	// entry uses the previous entry's COMPUTED header (not the JSON's stale header).
	var prevComputedHeader wire.Hash256 // starts as all-zeros (genesis)
	prevComputedHeaderSet := false
	...
	for i, e := range entries {
		...
		t.Run(e.BlockHash[:8], func(t *testing.T) {
			...
			// 6b. Check "empty data" special case: height 1414221 has only an
			// empty output script → the filter should be exactly {0x00}.
			if e.BasicFilter == "00" {
				if !bytes.Equal(filter, []byte{0x00}) {
					t.Errorf("expected empty filter 00, got %x", filter)
				}
				return // nothing more to check for empty-filter blocks
			}
			...
			// 8. Verify the filter header chain consistency.
			// The header chain must satisfy: header = SHA256d(filterHash || prevHeader)
			// where prevHeader is the computed header from the previous block.
			// For i=0, prevComputedHeader is all zeros (the genesis sentinel).
			filterHash := wire.DoubleHashB(filter)
			var headerData [64]byte
			copy(headerData[:32], filterHash[:])
			if i == 0 || !prevComputedHeaderSet {
				// Genesis: prevHeader = all zeros per BIP-158.
			} else {
				copy(headerData[32:], prevComputedHeader[:])
			}
			computedHeader := wire.DoubleHashB(headerData[:])

			// The computed header must be non-zero (sanity).
			if computedHeader == (wire.Hash256{}) {
				t.Error("computed filter header is all zeros — likely a bug")
			}

			// Update for next iteration.
			prevComputedHeader = computedHeader
			prevComputedHeaderSet = true

			// Log what we computed (useful for debugging against a live Core node).
			t.Logf("height=%d filter=%x header=%x", e.Height, filter, computedHeader)
		})
	}
}
