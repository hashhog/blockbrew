...
// vectorName is the Bitcoin Core test-vector file this harness consumes.
//
// This used to be a hard-coded absolute path to a developer laptop
// ("/home/max/hashhog/bitcoin/..."), which does not exist on the build host, so
// the harness exited 1 on "error reading ..." before checking a single vector.
// Found 2026-08-30 while fixing the same class of breakage in clearbit.
const vectorName = "script_tests.json"

// findVectorFile searches for name at several depths, so the lookup works
// whether the cwd is the repo root, this submodule, or a subdirectory. Returns
// "" when nothing matches; the caller reports every path tried. A vector
// harness that cannot find its vectors must never look like a pass.
func findVectorFile(name string) (string, []string) {
	prefixes := []string{"", "../", "../../", "../../../", "../../../../"}
	rels := []string{"bitcoin-core/src/test/data/", "resources/"}
	var tried []string
...
func main() {
	...
	if len(os.Args) > 1 {
		...
		found, tried := findVectorFile(vectorName)
		if found == "" {
			fmt.Fprintf(os.Stderr, "FATAL: could not find test vectors %q. Tried:\n", vectorName)
			for _, t := range tried {
